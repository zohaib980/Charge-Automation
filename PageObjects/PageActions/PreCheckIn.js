...
export class precheckinPageElements {
    ...
    basicInfoVerification(){
        ...
        cy.get(precheckinElementLocators.PreCheckInPageLocators.address).type('{downarrow}{enter}')
        // Wait for the dropdown to appear and select 1st item 
        // cy.get('.pac-container').should('be.visible');
        // cy.get('.pac-item:first').trigger('mouseover').click();
...
// Document verification
    ...
    contactInfo(){
        ...
        cy.get(':nth-child(7) > .gp-dl > dd')
            .should('contain', 'Station Road, Khand Bazar, Varachha, Surat, Gujarat, India')
